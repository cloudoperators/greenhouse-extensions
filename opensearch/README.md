...
| cluster.cluster.client | object | `{"service":{"annotations":{},"enabled":true,"externalIPs":[],"loadBalancerSourceRanges":["0.0.0.0/0"],"ports":[{"name":"http","port":9200,"protocol":"TCP","targetPort":9200}],"type":"ClusterIP"}}` | Configuration for the client service used to expose client nodes externally. |
...
| cluster.cluster.client.service.loadBalancerSourceRanges | list | `["0.0.0.0/0"]` | List of allowed IP ranges for external access when service type is `LoadBalancer`. |
