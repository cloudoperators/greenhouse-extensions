...
| cluster.snapshotLifecycle.clusterHost | string | `""` | OpenSearch URL the Job calls. Leave empty to default to the in-cluster Service rendered by the operator (named after `general.serviceName`, otherwise `cluster.cluster.name`, otherwise the release name; HTTPS port 9200). |
