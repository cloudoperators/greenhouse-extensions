...
| perses.config.plugin | object | <pre>plugin:<br>  disabled:<br>    - ClickHouse<br>    - GreptimeDB<br>    - Jaeger<br>    - Loki<br>    - Pyroscope<br>    - Splunk<br>    - Tempo<br>    - VictoriaLogs</pre> | Runtime plugin filtering (requires perses >= v0.54.0). Greenhouse only needs the Prometheus, Alertmanager and OpenSearch datasources, so all other datasource modules are disabled by module name. Panels, variables and explorers from other modules stay enabled so dashboards keep rendering. `enabled` and `disabled` are mutually exclusive. Feature reference: https://github.com/perses/perses/pull/4084 |
