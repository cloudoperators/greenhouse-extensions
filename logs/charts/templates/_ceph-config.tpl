  ...
  log_statements:
    - context: log
      ...
      statements:
        ...
        - merge_maps(log.attributes, ExtractGrokPatterns(log.body, "%{WORD:log_level}%{SPACE}%{NOTSPACE}%{SPACE}%{NOTSPACE:process.name}", true),"upsert")
        - set(log.attributes["network.protocol.name"], ConvertCase(log.attributes["network.protocol.name"], "lower")) where log.cache["network.protocol.name"] != nil
        - set(log.attributes["config.parsed"], "ceph_rgw") where log.attributes["project.id"] != nil
        - set(log.attributes["config.parsed"], "ceph_rgw") where log.attributes["log_level"] != nil
  ...
  log_statements:
    - context: log
      ...
      statements:
        ...
        - merge_maps(log.attributes, ExtractGrokPatterns(log.body, "%{WORD:log_level}%{SPACE}%{NOTSPACE}%{SPACE}%{SPACE}%{NOTSPACE:process.name}", true),"upsert")
