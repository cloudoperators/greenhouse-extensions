...
| openTelemetry.externalCollector.externalHttpConfig | object | `{"enabled":false,"forwardedBy":"external-http","path":"/audit/external","port":1517,"tls":{"enabled":true}}` | Activates the HTTP-JSON receiver for Logstash/fluent-bit-style pushers. Records land in the audit stream (audit-datastream / auditKafkaTopic) reusing the syslog audit exporters. Multiple senders may target this endpoint; each should set its own `forwarded_by`. |
...
| openTelemetry.externalCollector.externalHttpConfig.path | string | `"/audit/external"` | HTTP path the receiver serves. Sender must POST here. Expects one JSON object per request; a JSON array or newline batch is accepted but stored as a single record (not split per element). |
