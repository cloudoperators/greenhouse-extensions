...
| openTelemetry.externalCollector.externalHttpConfig | object | `{"enabled":false,"forwardedBy":"external-http","kafkaBrokers":[],"kafkaTopic":"audit","maxRequestBodySize":10485760,"path":"/audit/external","port":1517,"tls":{"enabled":true}}` | HTTP-JSON receiver for Logstash/fluent-bit-style pushers. In Kafka mode records go to externalHttpConfig.kafkaTopic; in non-Kafka mode to the syslog audit OpenSearch failover (audit-datastream). |
...
| openTelemetry.externalCollector.externalHttpConfig.kafkaBrokers | list | `[]` | Kafka brokers list used by external http kafka exporter. |
...
| openTelemetry.externalCollector.syslogConfig.kafkaBrokers | list | `[]` | Kafka brokers list used by syslog kafka exporter. |
